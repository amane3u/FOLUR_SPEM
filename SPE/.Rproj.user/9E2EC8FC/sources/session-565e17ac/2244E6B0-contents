...
dev.off()
colnames(country_information_CORN)


mean_elas <- country_information_CORN %>%
  full_join(country_information_RICE, by = c(
    "iso3", "demand_elas", "supply_elas", "demand_q", 
    "supply_q", "Production_USD_t", "SPAM_code"
  )) %>%
  full_join(country_information_SOYB, by = c(
    "iso3", "demand_elas", "supply_elas", "demand_q", 
    "supply_q", "Production_USD_t", "SPAM_code"
  )) %>%
  full_join(country_information_WHEA, by = c(
    "iso3", "demand_elas", "supply_elas", "demand_q", 
    "supply_q", "Production_USD_t", "SPAM_code"
  ))

mean_elas_summary <- mean_elas %>%
  select(iso3, demand_elas, supply_elas) %>%
  group_by(iso3) %>%
  summarise(
